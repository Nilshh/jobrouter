...
class StepstoneScraper(BaseScraper):
    ...
    def _parse_jobs(self, html):
        ...
        try:
            ...
            job_listings = soup.find_all("a", {"data-test-id": "job-item"})
            
            if not job_listings:
                job_listings = soup.find_all("article", class_="resultListItemBase")
            ...
            for item in job_listings:
                try:
                    ...
                    title_elem = item.find("h2") or item.find("a", class_="jobTitle")
                    title = title_elem.get_text(strip=True) if title_elem else ""
                    
                    # Versuche Unternehmen zu extrahieren
                    company_elem = item.find("span", class_="company") or item.find("div", class_="company-name")
