...
async function loadCities() {
    try {
        ...
        if (data.success && data.cities) {
            ...
            // Füge Städte hinzu (außer Deutschland)
            data.cities.forEach(city => {
                if (!Array.from(locationSelect.options).some(opt => opt.value === city)) {
...
if (scrapeForm) {
    scrapeForm.addEventListener('submit', async (e) => {
        ...
        console.log('Starte Scraping:', { query, location, radius });
    ...
    try {
        ...
        if (data.success && data.jobs && data.jobs.length > 0) {
            ...
            data.jobs.forEach(job => {
                platformCounts[job.Plattform] = (platformCounts[job.Plattform] || 0) + 1;
            });
            ...
            const table = `
                <div class="mb-3">
                    <small>Insgesamt: <strong>${data.count} Jobs</strong> | ${platformSummary}</small>
                </div>
